...
\def\sasp{\hspace*{26pt}}

\begin{minipage}{\textwidth}
\begin{multicols}{2}
\raggedcolumns
\noindent The different classes of states are:\\
\sasp\verb|> | -- the start state\\
\sasp\verb|< | -- any state except the start state\\
\sasp\verb|$ | -- all states\\
\sasp\verb|% | -- final states\\
\sasp\verb|@ | -- any state except final states\\
\sasp\verb|<>| -- any except start and final (middle)

\columnbreak

\noindent The different kinds of embeddings are:\\
\sasp\verb|~| -- to-state actions (\verb|to|)\\
\sasp\verb|*| -- from-state actions (\verb|from|)\\
\sasp\verb|/| -- EOF actions (\verb|eof|)\\
\sasp\verb|!| -- error actions (\verb|err|)\\
\sasp\verb|^| -- local error actions (\verb|lerr|)\\
\end{multicols}
\end{minipage}
