...
std::ostream &AsmCodeGen::TRANSITIONS()
{
	...
	for ( TransApSet::Iter trans = redFsm->transSet; trans.lte(); trans++ ) {
		...
		/* Destination state. */
		if ( trans->p.action != 0 && trans->p.action->anyCurStateRef() )
			out << "_ps = " << vCS() << ";";
		out << vCS() << " = " << trans->p.targ->id << "; ";

		if ( trans->p.action != 0 ) {
			/* Write out the transition func. */
			out << "goto f" << trans->p.action->actListId << ";\n";
		}
		else {
...
std::ostream &AsmCodeGen::TRANS_GOTO( RedTransAp *trans, int level )
{
	if ( trans->p.action != 0 ) {
		...
		out << "	jmp	.L_tr_" << trans->p.id << "\n";
	}
	else {
		/* Go directly to the target state. */
		//out << TABS(level) << "goto st" << trans->targ->id << ";";

		out << "	jmp	.L_st_" << trans->p.targ->id << "\n";
	}
	return out;
}
...
void AsmCodeGen::setLabelsNeeded()
{
	...
	if ( useAgainLabel() ) {
		...
		for ( TransApSet::Iter trans = redFsm->transSet; trans.lte(); trans++ ) {
			...
			if ( trans->p.action != 0 ) {
				/* Loop the actions. */
				for ( GenActionTable::Iter act = trans->p.action->key; act.lte(); act++ ) {
